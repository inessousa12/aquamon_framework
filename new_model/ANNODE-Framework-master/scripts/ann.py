...
import io_data
from cfg_validator import validate_ann_cfg
from io_data import get_model_name
...
def main():
    ...
    # 1 Args -> Create model & train it
        ...
        if os.path.isfile(data_path[0]):
            try:
                data = io_data.load_processed(data_path)
...
def get_cfg(ann_cfg_path):
    if os.path.isfile(ann_cfg_path):
        ann_cfg = io_data.load_cfg(ann_cfg_path)
        if validate_ann_cfg(ann_cfg):
            return ann_cfg
        else:
            print("Invalid cfg.")
...
def calculate_cdf(model, ann_cfg, i):
    if ann_cfg["cdf_data_path"] is not None:
        if os.path.isfile(ann_cfg["cdf_data_path"]):
            try:
                data = io_data.load_processed([ann_cfg["cdf_data_path"]])
            except IOError or ValueError:
...
def train_model(model, data, ann_cfg):
    ...
    while True:
        checkpoint_path = ann_cfg["checkpoint_path"] + get_model_name(ann_cfg) + "/run_" + str(run_n) + "/"
        try:
            os.makedirs(checkpoint_path)
            io_data.save_train_cfg(ann_cfg, checkpoint_path + "train.json")
            break
        except OSError:
            train_cfg = io_data.load_cfg(checkpoint_path + "train.json")
    ...
    model_path = ann_cfg["model_save_path"] + get_model_name(ann_cfg) + "/"
    ...
    io_data.save_loss(results, checkpoint_folder + "/loss.json")
...
def save_models(ann_cfg):
    ...
    for i in range(len(folders_found)):
        ...
        io_data.save_data(cdf, save_folder + "\\cdf.npz")
        io_data.save_loss(loss, save_folder + "\\loss.json")
