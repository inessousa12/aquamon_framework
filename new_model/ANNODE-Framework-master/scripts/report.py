...
from sensor_data import PredictionBlock
from format_data import generate1
from io_data import load_processed, load_raw
...
def start_report(report_data, threshold):
    ...
    start = datetime2matlabdn(datetime.strptime("2013-10-03 00:00", "%Y-%m-%d %H:%M"))
    end = datetime2matlabdn(datetime.strptime("2013-12-22 00:00", "%Y-%m-%d %H:%M"))

    os.makedirs(f"./report/{int(threshold * 10000)}/", exist_ok=True)
    files = listdir(f"./report/{int(threshold * 10000)}/")
    ...
    for i in range(len(snames)):
        ...
        predictions_file = f"./report/{snames[i]}_predictions.npy"
        if isfile(predictions_file):
            ...
            new_times.append(format_data.build_new_times(copy_sizes, copy_times, 0, 750))
            ...
            for nt in new_times[i]:
                ...
                input, input_times = generate1(nt[0][0], copy_sizes, copy_times, copy_values, 750, 0, 60, 20,
                                               new_times[i])
        ...
        np.save(f"./report/{int(threshold * 10000)}/{snames[i]}_faults{int(threshold * 10000)}.npy",
                quality[snames[i]])

    for i in range(len(snames)):
        values_y, values_x = values[i], times[i]
        corrected_values_y = corrected_values[i]
        values_x = [datetime.fromordinal(int(i)) + timedelta(days=i % 1) - timedelta(days=366) for i in values_x]
        ...
        plt.savefig(f'./report/{int(threshold * 10000)}/{snames[i]}.png',
        ...
        plt.gca().spines['right'].set_visible(False)
        plt.gca().spines['top'].set_visible(False)
        plt.savefig(f'./report/{int(threshold * 10000)}/{snames[i]}_corrected.png',
                    bbox_inches='tight', dpi=150, block=False)
        plt.clf()
        ...
        plt.gca().spines['right'].set_visible(False)
        plt.gca().spines['top'].set_visible(False)
        plt.savefig(f'./report/{int(threshold * 10000)}/{snames[i]}_quality.png',
                    bbox_inches='tight', dpi=150, block=False)
        plt.clf()
    ...
    with open(f'./report/{int(threshold * 10000)}/report{int(threshold * 10000)}.json', 'w') as file:
